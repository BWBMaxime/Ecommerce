...
<div id="<?=$product->id() ?>" class="product max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 py-16">
  <div class="flex flex-col md:flex-row -mx-4">
    ...
    <!-- Name of product -->
    <div class="md:flex-1 px-4">
      <h2 class="w-96 mb-3 leading-tight tracking-tight font-light text-3xl md:text-3xl color26"><?= $product->name() ?></h2>
      ...
      <div class="flex items-center space-x-4 my-4">
        <div>
          <div class="rounded-lg bg-gray-100 flex py-2 px-3">
            ...
            <span class="color4C text-2xl"><?= $product->displayPrice(true) ?></span>
          </div>
        </div>
        <div class="flex-1">
          ...
          <? else: ?>
            <p class="text-red-900 font-bold text-sm ml-4 mt-1">Out of Stock</p>
          <? endif ?>
        </div>
      </div>

      <div class="w-80 color4c my-8">
        <?= $product->shortDescription() ?>
        <? if ($product->isLongDescription()): ?>
        ... <a href="#description" class="font-bold hover:underline">see more</a>
        <? endif ?>
      </div>
      ...
      <div class="flex py-4 space-x-4">
        ...
        <!-- Button Cart-->
        <button id="description" type="button" class="add h-16 px-16 py-5 backColor4c text-white rounded-md focus:outline-none focus:ring-2 ring-gray-300 ring-offset-2 flex items-center hover:text-gray-300">
          Add to Cart
